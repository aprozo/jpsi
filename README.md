...
The first start takes a few minutes while the container is built. Stop the codespace when you are done to save your quota.

**Binder.** The [Binder button](https://mybinder.org/v2/gh/aprozo/jpsi/HEAD) builds the same environment from [environment.yml](environment.yml). The first build after a change to the repository can take 10 to 20 minutes; later launches are faster.
